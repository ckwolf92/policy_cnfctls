...
disp('I am solving the model under the optimal policy rule.')

get_optpol_modelsolve
...
shocks_var_all = zeros(T,T);
for t = 1:T
    shocks_var_all(t,1) = shock_optpol(t);
end
...
hold on
plot(0:1:IRF_plot,scale*y_s_optpol(1:IRF_plot+1),'linewidth',5,'linestyle','-','color',settings.colors.blue)
hold on
plot(0:1:IRF_plot,scale*y_s_base(1:IRF_plot+1),'linewidth',5,'linestyle',':','color',settings.colors.dgrey)
hold on
plot(0:1:IRF_plot,scale*y_s_optpol_pred(1:IRF_plot+1),'linewidth',5,'linestyle','--','color',settings.colors.black)
hold on
set(gcf,'color','w')
...
set(gca,'TickLabelInterpreter','latex')
hold on
plot(0:1:IRF_plot,scale*pi_s_optpol(1:IRF_plot+1),'linewidth',5,'linestyle','-','color',settings.colors.blue)
hold on
plot(0:1:IRF_plot,scale*pi_s_base(1:IRF_plot+1),'linewidth',5,'linestyle',':','color',settings.colors.dgrey)
hold on
plot(0:1:IRF_plot,scale*pi_s_optpol_pred(1:IRF_plot+1),'linewidth',5,'linestyle','--','color',settings.colors.black)
hold on
set(gcf,'color','w')
title('Inflation','interpreter','latex','fontsize',21)
xlabel('Horizon','interpreter','latex','FontSize',18)
legend({'True Optimum','Baseline','Predicted Optimum'},'Location','Northeast','fontsize',18,'interpreter','latex')
grid on
hold off
...
hold on
for i = 1:40
    plot(0:1:IRF_plot,scale*shocks_var_all(i,1:IRF_plot+1),'linewidth',5,'linestyle','-','color',settings.colors.var(i,:))
    hold on
end
set(gcf,'color','w')
